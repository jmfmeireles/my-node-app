...
const seedDatabase = async () => {
  try {
    ...
    const profiles = await Profile.bulkCreate([
      {
        biography: "John is a seasoned writer with a passion for adventure.",
        authorId: authors[0]!.dataValues.id,
      },
      {
        biography: "Jane specializes in mystery novels and thrillers.",
        authorId: authors[1]!.dataValues.id,
      },
      {
        biography: "Emily writes about science and exploration.",
        authorId: authors[2]!.dataValues.id,
      },
    ]);
    console.log(
      "Profiles seeded:",
      profiles.map((profile) => profile.toJSON())
    );
    ...
    console.log(
      "Shelves seeded:",
      shelves.map((shelf) => shelf.toJSON())
    );
