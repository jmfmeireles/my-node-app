...
describe('Movies Service', () => {
  ...
  describe('updateMovie', () => {
    it('should return null if movie not found', async () => {
      ...
      const result = await MoviesService.updateMovie('507f1f77bcf86cd799439011', { title: 'Updated' });
    ...
    it('should update movie successfully', async () => {
      ...
      const result = await MoviesService.updateMovie(movieId.toString(), { title: 'Updated Movie' });
  ...
  describe('deleteMovie', () => {
    it('should throw an error if movie not found', async () => {
      ...
      await expect(MoviesService.deleteMovie('507f1f77bcf86cd799439011')).rejects.toThrow('Movie not found');
    });
    ...
    it('should delete movie without comments', async () => {
      ...
      mockedDb.collection = jest.fn((collectionName: string) => {
        ...
        if (collectionName === 'comments') {
          return { ...mockCollection, deleteMany: jest.fn().mockResolvedValue({ deletedCount: 0 }) };
        }
        return mockCollection;
      }) as any;
    ...
    it('should delete movie with associated comments', async () => {
      ...
      mockedDb.collection = jest.fn((collectionName: string) => {
        ...
        if (collectionName === 'comments') {
          return { ...mockCollection, deleteMany: jest.fn().mockResolvedValue({ deletedCount: 5 }) };
        }
        return mockCollection;
      }) as any;
