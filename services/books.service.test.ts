...
describe('Books Service', () => {
  ...
  describe('createBook', () => {
    ...
    it('should create a book successfully without shelves', async () => {
      ...
      const result = await BookService.createBook({ title: 'Test Book', publicationYear: 2024, authorId: 1 });
    ...
    it('should create a book with shelves', async () => {
      ...
      expect(result).toEqual(mockBookWithShelves);
      expect(mockBook.addShelves).toHaveBeenCalledWith([mockShelf1, mockShelf2], { transaction: mockTransaction });
      expect(mockTransaction.commit).toHaveBeenCalled();
    });
    ...
    it('should rollback if one or more shelves not found', async () => {
      ...
      await expect(
        BookService.createBook({ title: 'Test Book', publicationYear: 2024, authorId: 1, shelfIds: [1, 999] })
  ...
  describe('fetchAllBooks', () => {
    it('should return all books with shelves', async () => {
      ...
      expect(mockedBook.findAll).toHaveBeenCalledWith({ include: [{ model: Shelf, as: 'shelves' }] });
    });
  });
  ...
  describe('fetchBookById', () => {
    it('should return a book by id', async () => {
      ...
      expect(mockedBook.findByPk).toHaveBeenCalledWith('1', { include: [{ model: Shelf, as: 'shelves' }] });
    });
  });
  ...
  describe('updateBook', () => {
    it('should return null if book not found', async () => {
      ...
      const result = await BookService.updateBook('1', { title: 'Updated', publicationYear: 2024, authorId: 1 });
    ...
    it('should update a book successfully', async () => {
      ...
      mockedBook.findByPk.mockResolvedValueOnce(mockBook as never).mockResolvedValueOnce(mockUpdatedBook as never);
      mockedAuthor.findByPk.mockResolvedValue(mockAuthor as never);

      const result = await BookService.updateBook('1', { title: 'New Title', publicationYear: 2024, authorId: 1 });

      expect(result).toEqual(mockUpdatedBook);
      expect(mockBook.update).toHaveBeenCalledWith({ title: 'New Title', publicationYear: 2024, authorId: 1 }, { transaction: mockTransaction });
      expect(mockTransaction.commit).toHaveBeenCalled();
    });
  });
  ...
  describe('removeBookFromShelf', () => {
    it('should throw an error if book or shelf not found', async () => {
      ...
      await expect(BookService.removeBookFromShelf('1', '1')).rejects.toThrow('Book or Shelf not found');
    });

    it('should throw an error if book is not on the shelf', async () => {
      const mockBook = { id: 1, hasShelf: jest.fn().mockResolvedValue(false), removeShelf: jest.fn() };
      const mockShelf = { id: 1 };

      mockedBook.findByPk.mockResolvedValue(mockBook as never);
      mockedShelf.findByPk.mockResolvedValue(mockShelf as never);

      await expect(BookService.removeBookFromShelf('1', '1')).rejects.toThrow('Book is not on the specified shelf');
    });

    it('should remove book from shelf successfully', async () => {
      const mockBook = { id: 1, hasShelf: jest.fn().mockResolvedValue(true), removeShelf: jest.fn() };
      const mockShelf = { id: 1 };
  ...
  describe('fetchBookShelves', () => {
    ...
    it('should return book shelves', async () => {
      const mockShelves = [{ id: 1, name: 'Fiction' }, { id: 2, name: 'Classics' }];
