...
describe('Auth Service', () => {
  ...
  describe('registerUser', () => {
    it('should throw an error if password is undefined', async () => {
      await expect(registerUser({ username: 'testuser', password: undefined } as unknown as Credentials)).rejects.toThrow('Password must be at least 6 characters long');
    });

    it('should throw an error if password is too short', async () => {
      await expect(registerUser({ username: 'testuser', password: '12345' })).rejects.toThrow('Password must be at least 6 characters long');
    });

    it('should register a user successfully with a valid password', async () => {
      const hashedPassword = 'hashed_validpassword';
      mockedBcrypt.hash.mockResolvedValue(hashedPassword as never);
      mockedUser.create.mockResolvedValue({ id: 1, username: 'testuser', password: hashedPassword } as any);
      ...
      expect(mockedUser.create).toHaveBeenCalledWith({ username: 'testuser', password: hashedPassword });
    });
  });
  ...
  describe('loginUser', () => {
    it('should throw an error if user not found', async () => {
      ...
      await expect(loginUser({ username: 'nonexistent', password: 'password' })).rejects.toThrow('User not found');
    });
    ...
    it('should throw an error if password is wrong', async () => {
      ...
      await expect(loginUser({ username: 'testuser', password: 'wrongpassword' })).rejects.toThrow('Password is wrong');
    });
